...
ToDo [NA in tidyverse](https://pine.do/R/network-analysis-in-the-Tidyverse.html)


ToDo Random Networks in der Organisationsforschung [Phil Chapman](https://chapmandu2.github.io/orgsurveyr_docs/articles/organisations_with_ggraph.html)
...
knoten <- tibble(name=c("A","B","C","D","E","F","G","H"),
                 type=c("Alter","Alter","Alter","Alter","Alter","Alter","Alter","Ego"))
...
```{r}
tg %>% 
    activate(nodes) %>%   #Knoten aktivieren
    mutate(degree = centrality_degree()) # Grad-Zentralität berechnen&speichern
```
...
* Betweenness-Zentralität und Homophilie

```{r}
tg %>% activate(edges) %>% # Kanten aktivieren
              mutate(betweenness = centrality_edge_betweenness(), 
                     homo = (.N()$sex[from] == .N()$sex[to])) 
# Obs! .N() verwendet Daten der an die aktivierten Kanten "grenzenden" Knoten 
```
...
flo_tidy %>% 
  ...
  ggraph("stress",bbox=10) + 
    ...
    scale_size(range = c(4,10)) + theme_graph()
```
...
## Durchmesser am Beispiel "Freundschaftsnetzwerk"
