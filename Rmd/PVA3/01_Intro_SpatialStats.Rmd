...
### Teil 1: Räumliche Statistik und Geostatistik
...
## Definitionen räumlicher Nachbarschaft


.pull-left[

"Klassisch": Gemeinsame Grenze/Berührungspunkt

```{r}
#| echo: false
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","arealdata-neighborscontiguity_(Moraga).png"))
```
...
Abstand (bspw. euklidische Distanz, Manhattan-Distanz,...)
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","arealdata-neighborsdistance_(Moraga).png"))
```

]
.quelle[Abbildungen: `r Citet(bib,"moraga_spatial_2023")`]


???


* Schachfiguren: Rook (Turm - darf nur gerade), Queen (Dame, darf auch diagonal)
...
## Definitionen räumlicher Nachbarschaft (cont'd)
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","arealdata-neighborskneighbors_(Moraga).png"))
```
] 
--
.pull-right[
* erste vs. höhere Ordnung

```{r}
#| echo: false
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","arealdata-orderk_(Moraga).png"))
```
]
...
## Nachbarschaft, Ähnlichkeit und Abhängigkeit
...
* um das Spiel erfolgreich zu meistern, wird die Eigenschaft der Ähnlichkeit benachbarter Teile ausgenutzt
...
## Formen der räumlichen Autokorrelation
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","arealdata-SAC_(Moraga).png"))
```


???

* Auto: self - es geht um dieselbe Eigenschaft (eine Variable bzw. Ausprägung)

* Correlation: degree of relative correspondence (Korrelation zeigt an, ob Muster vorhanden sind)
...
## Definition (nach D. Griffith)

.blockquote[
"[Spatial autocorrelation] may be defined as the correlation among values of [i.e., within]
a single variable strictly attributable to their
relatively close locational positions on a twodimensional
surface, introducing a deviation
from the independent observations assumption
of classical statistics. Spatial autocorrelation
exists because real world phenomena are typified
by orderliness, (map) pattern, and systematic
concentration, rather than randomness."
.tr[
`r Citet(bib, "griffith_spatial_2017")`, S. 2087
]
]
...
* Ausgangspunkt: Wahrer Wirkungszusammenhang linear

$$y=X\beta+\varepsilon$$

* Methode der kleinsten Quadrate (Ordinary Least Squares, OLS) zur Ermittlung der Schätzer $\hat{\beta}$ (für die wahren Parameter $\beta$)
...
Unter den getroffenen Annahmen (ohne b4) sind die nach OLS-Methode ermittelten Schätzer $\hat{\beta}$ innerhalb der Klasse der unverzerrten linearen Schätzer effizient.
...
* Verletzung der Annahme `r fa('circle-right')` verzerrte Schätzer und fehlerhafte p-Werte
...
## Arten von Koordinatenreferenzsystemen (CRS)

* Geodätische CRS

  * Geographische CRS (2D, 3D)
...
* Geodätisches CRS: Geographische und Geozentrische CRS werden auch als Geodätisches CRS bezeichnet.
  * Geographisches CRS: Das geografische CRS verwendet ein **ellipsoidisches Modell der Erde**, um auf dessen Oberfläche die Position eines Objekts
      * mit **zweidimensionalen Koordinaten (Längen- und Breitengrad)** zu beschreiben. Die Lage des künstlichen Ellipsoides bezüglich der realen Erde wird mit einem geodätischen Datum beschrieben.
      * Wird *zusätzlich eine Höhe** angegeben, die auf das Ellipsoid bezogen ist, wird das als **3D Geographisches CRS** bezeichnet.
  * Geozentrisches CRS: Das geozentrische CRS geht vom **Schwerpunkt der Erde als Nullpunkt** aus und verwendet **dreidimensionale Koordinaten**, um die Position eines Objektes zu beschreiben.
...
## Geographische vs. projizierte CRS

.panelset[
.panel[.panel-name[Geographisch]
```{r}
#| echo: false
#| out-width: "80%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","geographicCRS_(Moraga).png"))
```

]
.panel[.panel-name[Projiziert]
```{r}
#| echo: false
#| out-width: "80%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","projectedCRS_(Moraga).png"))
```
]
]
.quelle[Bildquelle: `r Citet(bib,"moraga_spatial_2023")`.]
...
## Grundlegende Projektionstypen

```{r}
#| echo: false
#| out-width: "80%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","projection_families.png"))
```

.quelle[Quelle: [docs.qgis.org](https://docs.qgis.org/3.34/en/docs/gentle_gis_introduction/coordinate_reference_systems.html#the-three-families-of-map-projections).]
...
## Wichtige Koordinatensysteme für die Praxis


.panelset[
.panel[.panel-name[Übersicht]
* Die meisten gebräuchlichen CRS können durch Angabe ihrer EPSG-Codes (European Petroleum Survey Group) oder ihrer Proj4-Strings angegeben werden.

* Übersicht: [Spatial Reference List](https://spatialreference.org/ref/)

* Besonders häufig verwendet: 
  * WGS 84
  * ETRS89 / UTM
]
.panel[.panel-name[Video WGS84]
<iframe width="800" height="400" src="https://www.youtube.com/embed/o2_t6MGieLI?si=NMpAhuaTnxxI63DL" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
]
]
...
* Einzelheiten zu einer bestimmten Projektion können mit der Funktion st_crs() des sf-Pakets eingesehen werden. Der EPSG-Code 4326 bezieht sich beispielsweise auf die WGS84-Längen-/Breitengradprojektion.
...
.blockquote[Entwicklungslinien der Analyse Räumlicher Daten]

.blockquote[Grundlegende Konzepte]
...
## Matrixdarstellung Nachbarschaft

```{r}
#| echo: false
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","Matrix_(Griffith).png"))
```
...
* Coding Schema abhängig vom Nachbarschafts-Konzept. Hier: Rook (TUrm)

  * Create indicator variables: IS location A adjacent to location B (1=yes)

  * There are more sophisticated coding strategies (length of common boundary, etc….)

* Premultiplying a vector of attributes with adj. Matrix

* Matrix C interpretation vs. Matrix w row-sum-standardized
...
* Moran's I, `r Citet(bib,"moran_notes_1950")`

$$MC=\frac{n}{\sum_{i=1}^n \sum_{j=1}^nc_{ij}}\cdot \frac{\sum_{i=1}^n \sum_{j=1}^nc_{ij}(y_i-\bar{y})(y_j-\bar{y})}{\sum_{i=1}^n(y_i-\bar{y})^2}$$



* Geary' C, `r Citet(bib, "geary_contiguity_1954")`

$$GR=\frac{n-1}{2\cdot\sum_{i=1}^n \sum_{j=1}^nc_{ij}}\cdot \frac{\sum_{i=1}^n \sum_{j=1}^nc_{ij}(y_i-y_j)^2}{\sum_{i=1}^n(y_i-\bar{y})^2}$$

???

* Quantifizierung der Beziehungen  zwischen miteinander verbundenen Objektpaaren
im geografischen Raum


* Objektpaare in Adjazenzmatrix erfasst 

* c-Werte aus c-Matrix (Adjazenzmatrix, s.o.)
 cij commonly is 0 or 1, with 1 denoting that a linkage exists between
locations i and j (i.e., the pair is tied together) and 0 otherwise)

* Zwei der populärsten Maße für Autocorrelation 

* Beide Nenner: Varianz von y

* Zähler Moran: Produkt-Momente ähnlich wie Bravais-Pearson-Korrelation. Unterschied: Multiplikatuion mit $c_{ij}$

* Zähler Geary: Paarweise vergleiche (statt Produkt-Momente)

+ for nominal data: join counts
...
## Mathematische Beschreibung räumlicher Autokorrelation: Autoregression

* Bedingte Autoregression (CAR): Kovarianzmatrix $\sum$:

$$\sum=(I-\rho W) ^{-1}\sigma^2$$

* Simultane Autoregression (SAR): Kovarianzmatrix $\sum$:

$$\sum=[(I-\rho W^T)(I-\rho W)]^{-1}\sigma^2$$

Ausführlicher: vgl. `r Citet(bib, "griffith_spatial_2017")`


???

* Because spatial autocorrelation refers to observations **within a single variable**, given the **identically
distributed** statistical assumption, each observation has the same population variance. 

* Because observations are pairwise correlated,an n-by-n covariance matrix, say $\sum$, can be constructed
for them, with the same sequence of observations used to label its rows and columns
...
## Visualisierung räumlicher Autokorrelation

.pull-left[
.center[Moran Streudiagramm]
<br>
```{r}
#| echo: false
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","Moran_scatterplot_(Barreca_etal_2017).png"))
```
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(xfun::from_root("img","PVA3","schematic_variogram_(wikipedia).svg"))
```


.quelle[Bildquelle: links: `r Citet(bib,"barreca_assessing_2017")`, rechts: [wikipedia.org](https://en.wikipedia.org/wiki/Variogram).]
...
Todo Griffith (2017, S. 2093ff) Interpretation räumlicher Autokorrelation
...
* Explorative Analysen
  ...
  * Beschreibung der Daten (Karten, Kennzahlen)
  
  * Muster aufdecken und Hypothesen generieren
  
* Statistische Inferenz

  * Robuste Regressionsanalysen unter Berücksichtigung von Autokorrelation
...
## Grundidee Inferenz mit räumlichen Daten
...
## Zusammenfassender Überblick Räumliche Statistik vs. Geostatistik
