...
.blockquote[strings zerlegen (parsing)]
...
```{r}
library(glue)
name <- c("Fred","Fritz","Franz")
paste("Mein Freund heißt", name,".")
```
...
* Im Unterschied zur `paste()`-Funktion werden Zeichen ohne Leerzeichen verknüpft:
...
.panelset[
.panel[.panel-name[glue()-Funktion]

* `glue()`-Funktion grundsätzlich ähnlich wie die `paste()` und `paste0()`-Funktionen, aber schlankere Syntax:

```{r}
library(glue)
name <- c("Fred","Fritz","Franz")
glue("Mein Freund heißt {name}.")
```

`r fa('circle-right')` Einbinden von Objekten mit geschweifter Klammer

`r fa('circle-right')` Bessere Lesbarkeit, v.a. bei Verknüpfung vieler Objekte


]
.panel[.panel-name[glue_data()-Funktion]
...
library(magrittr)
cars <- mtcars %>% 
          tibble::rownames_to_column(var="Modell") 
head(cars, 3)
```
...
cars %>% 
  ...
  glue_data("Das Modell {Modell} hat {cyl} Zylinder und {hp} PS.")
```
]
.panel[.panel-name[glue_collapse()_Funktion]

* Möglichkeit zur Differenzierung der Separatoren mit der Option `last`

```{r}
clubs <- c("FCB", "BVB","B04","RB","VFB")
glue_collapse({clubs[1:3]}, sep = ", ", last = ", und ")
```
...
Erstes Beispiel: 
* anders als paste, 
  * wird die geschweifte Klammer für das Einbinden von Objekten genutzt
    * erhöht die Lesbarkeit (Übersichtlicher als Zahlreiche Anführungszeichen und Kommas)
  * außerdem: paste erzeugt eine Zeile, glue eine Zeile pro Vektorelement

* gute Quelle: [A. Rohan](https://rohanalexander.github.io/doss_toolkit_book/paste-paste0-glue-and-stringr.html)
...
## Exkurs: `parse()`-Funktion

* Ausdrücke aus *string*-Variablen zur möglichen Auswertung extrahieren:

```{r}
x <- "7*7" # Ausdruck als string
class(x)
```
--
.pull-left[

* Parsen des R-Ausdrucks
...
## Zeichenketten verknüpfen mit `str_glue()`

* Das **stringr**-Paket enthält wrapper-Funktionen der Funktionen im **glue**-Paket:

```{r}
tbl_babynames <- tbl_babynames %>%
                    mutate(Name = str_glue("Odermatt, {Vorname}")) 
...
* Als Alternative zur paste-Funktion

* hier: `mutate(Name = paste("Odermatt,", Vorname))`
...
Quelle zu regex [Ozan Jaquette](https://anyone-can-cook.github.io/rclass2/lectures/strings_and_regex/strings_and_regex.html)
...
## Identifikation von Zeichen mit `char_class()` und `negated_char_class()`
...
## Unicode als Text Encoding Standard

```{r}
#| echo: false
knitr::include_url('https://www.unicode.org/charts/',height = '470px')
```
...
* Stern zu finden in [Unicode-Chart](https://www.unicode.org/charts/PDF/U2600.pdf)
...
```{r}
str_view(normal, pattern = "\u03BC",html=TRUE)
```
]
--
.pull-right[

* Mehrere Zeichen einer "Gruppe"

```{r}
str_view(normal, rebus::greek_and_coptic(),html=TRUE)
```
]
